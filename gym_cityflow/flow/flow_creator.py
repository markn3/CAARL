...
def create_flow(lower_volume, upper_volume, num_intervals, interval_duration, total_steps, seed=0, distribution='normal'):
    ...
    if distribution == 'normal':
        ...
        std_dev_volume = (upper_volume - lower_volume) / 4  # rough estimate for standard deviation
    ...
    for i, volume in enumerate(volumes):
        for _ in range(volume):
            ...
            flow = {
                ...
                "route": route,
                "interval": int(interval_duration),
                "startTime": int(intervals[i]),
                "endTime": int(min(intervals[i] + interval_duration, total_steps))
            }
            flows.append(flow)
...
create_flow(lower_volume, upper_volume, num_intervals=int(num_intervals), interval_duration=int(interval_duration), total_steps=total_steps, seed=0)
